...
export default function WheelScreen({ navigation }) {
  ...
  const spinWheel = () => {
    ...
    const totalRotation = currentDeg + (numberOfRounds * 360) + targetDegree - (currentDeg % 360);
  ...
  return (
    <SafeAreaView style={styles.container}>
          ...
          {DESTINATIONS_POOL.map((item, i) => (
            <View key={i} style={styles.sectorWrapper}>
              ...
              <View style={[
                ...
                { transform: [{ rotate: `${i * sectorAngle + (sectorAngle / 2)}deg` }] }
              ]}>
                <Text 
                  numberOfLines={1}
                  style={[styles.sliceText, { color: (i === 2) ? '#1A3C40' : '#fff' }]}
                >
                  {item.name}
                </Text>
              </View>
      ...
      <View style={styles.footer}>
        <TouchableOpacity style={styles.spinBtn} onPress={spinWheel} disabled={isSpinning}>
          <Text style={styles.btnText}>{isSpinning ? 'מסתובב...' : 'סובב אותי!'}</Text>
        </TouchableOpacity>
        
        {result && !isSpinning && (
          <TouchableOpacity style={[styles.spinBtn, styles.nextBtn]} onPress={() => navigation.navigate('Home')}>
