...
namespace MatchingAppServer.Services
{
    ...
    public class AiItineraryService
    {
        ...
        private string BuildUserPrompt(Trip trip, int userA, int? userB, bool regenerate, List<string> previousTitles)
        {
            ...
            rules.Add("why: לכל פעילות כתוב משפט אחד (עד 15 מילים) שמנמק את הבחירה על בסיס העדפה ספציפית — תחום עניין, עדיפות מדורגת, קצב או אורח חיים — נסח בלשון משותפת (\"מתאים לכם\", \"בזכות תחום העניין המשותף...\") ובלי לייחס את ההעדפה לאחד המטיילים. אסורים נימוקים גנריים כמו \"מקום מומלץ\" או \"שווה ביקור\". אין נימוק אמיתי — השאר ריק (\"\").");
            ...
            // פרטיות ולשון: המטיילים מיוצגים בפרומפט כאנונימיים — אסור שהתוויות ידלפו לטקסט הגלוי.
            rules.Add("בכל הטקסט הגלוי למשתמש (summary, match_reason, planning_notes, why) אסור בהחלט להשתמש בתוויות או במספור של המטיילים — אסורים במפורש הביטויים \"מטייל/ת א'\", \"מטייל/ת ב'\", \"המשתמש הראשון\", \"המשתמש השני\" וכל וריאציה שלהם, וכן אסור להמציא שמות. פנה תמיד בלשון משותפת בלבד: \"אתם\", \"שניכם\", \"העדפותיכם המשותפות\".");
